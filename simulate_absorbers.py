...
def make_absorber_templates(N_total, z_min=1.0, z_max=4.0, output_dir='output/abs_templates', verbose=True):
    ...
    filelog.write(output_dir + "list_templates.csv", format='csv', overwrite=True)
    abslog = Table(abs_info_list)
    abslog.write(output_dir + "list_absorbers.csv", format='csv', overwrite=True)
    all_dlas = (abslog['LOG_NHI'] > 20.3) & (abslog['Z_ABS'] > 2.05)
    DLAlog = abslog[all_dlas]
    DLAlog.write(output_dir + "list_dlas.csv", format='csv', overwrite=True)
    if verbose:
        print("")
